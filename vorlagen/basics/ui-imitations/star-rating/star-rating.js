...
function main() {
  const builder = new StarRatingBuilder()
    ...
    .setDefaultColor("gold")
    .setDefaultRating(2.8)

  const items = [builder.build(), builder.build()]
  console.log(items)

  builder.setDefaultRating(4.3)
  items.push(builder.build())

  document.body.prepend(...items.map(i => i.container))
  const controllers = [...items.map(i => i.controller)]
