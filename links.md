## Timpstamp

[WebAPIs](https://developer.mozilla.org/en-US/docs/Web/API/Window/localStorage)
[working with Json](https://developer.mozilla.org/en-US/docs/Learn/JavaScript/Objects/JSON)
[working with API](https://developer.mozilla.org/en-US/docs/Web/API/Element)
[working with API](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Date)
[working with CSS calc()](https://developer.mozilla.org/en-US/docs/Web/CSS/calc-constant)



[working with pxel umrechnen](https://nekocalc.com/de/em-zu-px-umrechner)

[webFonts](https://gwfh.mranftl.com/fonts)


[Entwickler-Online-Developement](https://jsfiddle.net/boilerplate/react-jsx)


[PSEUDO-Elements](https://www.tjvantoll.com/2013/04/15/list-of-pseudo-elements-to-style-form-controls/)


[Git-Hub-Pages erstellen](https://www.youtube.com/watch?v=BT4WzyT2g8k)
